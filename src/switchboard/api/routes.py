...
async def writer_emulation_page(request: Request):
    ...
    async with RunContext.open() as ctx:
        ...
        sel_brand = request.query_params.get("brand") or (keys[0] if keys else "")
    ...
    return templates.TemplateResponse(request, "writer_emulation.html",
        {"user": user, "brands": brands, "sel_brand": sel_brand, "writers": writers,
         "max_norm": max_norm, "profiles": profiles, "window": window,
         "top_count": top_count,
         "may_edit": user.get("role") in ("global_admin", "portfolio_admin")})
